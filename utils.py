...
def mse(real_value, pred_value):
    ...
    size = len(real_value)
    for i in range(size):
        squared_error += (pred_value[i] - real_value[i]) ** 2
...
def plots(x_values, y_values, metrics, filepath, dataset, col):
    ...
    ax1.hist(x_values, bins=10, color=col)
    ax1.set_xlim(mini, maxi)

    ax2 = plt.subplot2grid((2, 3), (1, 2))
    ax2.set_title('Prediction distrubution')
    ax2.hist(y_values, bins=10, color=col)
    ax2.set_xlim(mini, maxi)

    ax3 = plt.subplot2grid((2, 3), (0, 0), colspan=2, rowspan=2)
    ax3.set_title(dataset)
    ax3.scatter(x_values, y_values, s=5, color=col)
    ax3.set_xlabel('Real values')
    ax3.set_ylabel('Predicted values')
    ax3.set_ylim(mini - margin, maxi + margin)
    ax3.set_xlim(mini - margin, maxi + margin)
    ax3.plot([mini - margin, maxi + margin], [mini - margin, maxi + margin], 'r', label='Identity line')
    ax3.plot([mini - margin, maxi + margin], [a * (mini - margin) + b, a * (maxi + margin) + b],
             label='Trend line', ls='--', c=col)
    ax3.text(mini - 0.9*margin, maxi - 4*margin,'RMSE = {:.2f}\nMAE = {:.2f}\nPearson r = {:.2f}\nSpearman r = {:.2f}\nCI = {:.2f}\nTest loss = {:.2f}'.format(metrics[0] ,metrics[1], metrics[2], metrics[3], metrics[4], metrics[5]), fontsize = 12)
...
if __name__=="__main__":
    ...
    plots(a,b, [0 for _ in range(6)], 'data/', 'test', 'b')