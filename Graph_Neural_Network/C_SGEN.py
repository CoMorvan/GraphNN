...
def load_tensor(file_name, dtype):
    return [dtype(d).to(torch.device('cuda')) for d in np.load((file_name + '.npy'), allow_pickle=True)]
...
class C_SGEN(nn.Module):
    def __init__(self, k, C_SGEN_layers, batch, window, lr):
    ...
    def __call__(self, data_batch, std, mean, C_SGEN_layers, train=True):
        inputs = data_batch[:-1]
...
class Trainer(object):
    def __init__(self, model, std, mean, C_SGEN_layers, lr):
        self.model = model
